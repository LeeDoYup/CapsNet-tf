...
def squash(vector, axis=-1, epsilon=1e-5, name='squash_op'):
    with tf.name_scope(name) as scope:
        vec_norm_sq = tf.reduce_sum(
            tf.square(vector), axis=axis, keep_dims=True)
...
def conv2d(input_,
    ...
    with tf.variable_scope(name) as scope:
        ...
        print(w)
        conv = tf.nn.conv2d(
            input_, w, strides=[1, stride_h, stride_w, 1], padding=padding)
        print(conv)
        b = tf.get_variable(
        ...
        if 'primary' in self.name:  #Primary Capsule
            with tf.variable_scope(self.name) as scope:
                ...
                if len(input_shape) == 3:
                    input_ = tf.expand_dims(
                        input_, axis=-1)  #for gray scale error
            ...
            with tf.variable_scope(self.name) as scope:
                ...
                for i in range(route_iter):
                    ...
                    sum_s = tf.reduce_sum(
                        s, axis=1, keep_dims=True, name='weighted_sum')
                    capsules = squash(
                        sum_s, axis=-2
                    )  #(None, 1, # of nex capsule capsule, d_capsule, 1)
                    caps_out_tile = tf.tile(
                        capsules, [1, self.input_shape[1], 1, 1, 1],
